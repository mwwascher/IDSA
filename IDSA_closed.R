...
plot(I.count, main = "Infecteds", xlab = "day", ylab = "I", ylim = c(0,500))
...
lik = function(adj.mat, p.t, S.0)
{
  ...
  if (is.na(adj.mat[3]))
  {
    lik = 1 - (1-S.0) - sum(p.t[left.end:length(p.t)])  
  } else
  {
    if (is.na(adj.mat[2]))
    {
    ...
    if (adj.mat[3] == 0)
    {
      lik = (1-S.0)/(sum(p.t[left.end:length(p.t)]) + (1-S.0))
    } else
    {
      lik = p.t[adj.mat[3]]/((sum(p.t[left.end:right.end]))+add)
    }
  }
  lik = as.numeric(lik)
  return(lik)
...
{
  S.e = (1-(1-beta[1]/n)^I.0)*(1-phi.t[1])^(t-1)
  if (t > 1)
  {
    for (k in 1:(t-1))
    {
...
lik.e = function(inf.dates, ex.dates)
{
  dif = inf.dates - ex.dates
  dif = dif[!is.na(dif)]
  log.lik = sum(log(phi[1]^dif))
  return(log.lik)
}
...
p.t.null = p.fun(S.med, 1)#rep(inf.prop/n.days, n.days)n
S.null = .99
I = rep(0, n.days)
delta = rep(0, n.days)
eps = rep(0, n.days)
...
for (k in 1:nreps)
{
  ...
  I.0 = max(1, trunc(length(which(inf.dates == 0)) + n*X.unif))#trunc(rbeta(1,1+length(which(inf.dates == 0)/2), 199+n)*n)
  E.0 = max(1, trunc(length(which(ex.dates <= 0 & inf.dates > 0)) + n*X.unif))#trunc(rbeta(1,1+length(which(inf.dates == 0)/2), 199+n)*n)
  S.0 = 1 - (I.0+E.0)/n
  S.e.0 = 1 - I.0/n
  delta[1] = length(which(ex.dates == 1))
  omega[1] = length(which(inf.dates == 1))
  gamma[1] = rbeta(1,14,100)
  eps[1] = rbinom(1, I.0, gamma[1])
  I[1] = I.0 + omega[1] - eps[1]
  e.t[1] = E.0 + delta[1] - omega[1]
  S.t[1] = n - delta[1]
  beta[1] = I[1]/n*rbeta(1, delta[1] + .5, n - delta[1] + .5)
  S.prod[1] = (1 - beta[1]/n)^I.0
  S[1] = S.0*S.prod[1]
  S.e[1] = S.0*S.comp(beta, t=1, I, S, I.0, E.0, S.0, S.e.0)+(S.e.0-S.0)*phi.t[1]
  for (j in 2:(n.days))
  {
    delta[j] = length(which(ex.dates == j))
    eps[j] = rbinom(1, I[j-1], gamma[j-1])
    #eps[j] = length(which(test.mat[,2] == j-2))
    omega[j] = length(which(inf.dates == j))
    I[j] = I[j-1] + omega[j] - eps[j]
    e.t[j] = e.t[j-1] + delta[j] - omega[j]
    S.t[j] = S.t[j-1] - delta[j]
    beta[j] = n/I[j]*rbeta(1, delta[j] + .5, S.t[j-1] - delta[j] + .5)
    a.t = 3#.1*length(which(r.dates == j)) + 1
    b.t = 6#.1*max(1, I[j-1] - length(which(r.dates == j)))
    gamma[j] = rbeta(1, eps[j] + 10, I[j-1] - eps[j] + 50)
    S.prod[j] = (1 - beta[j-1]/n)^I[j-1]
    S[j] = S[j-1]*S.prod[j]
    S.e[j] = S.0*S.comp(beta, t=j, I, S, I.0, E.0, S.0, S.e.0)+(S.e.0-S.0)*phi.t[1]^(j)
  }
  p.t = p.fun(S.e, S.0)
...
fig.on <- fig.on %>% add_trace(x = c(1:(n.days-4)), y = I.med[1:(n.days-4)], mode = "lines", name = '50%')
fig.on <- fig.on %>% add_trace(x = c(1:(n.days-4)), y = I.min[1:(n.days-4)], mode = "lines", name = '2.5%', fill = "tonexty", fillcolor = "rgba(168, 216, 234, 0.5)")
fig.on <- fig.on %>% add_trace(x = c(1:(n.days-4)), y = I.max[1:(n.days-4)], mode = "lines", name = '97.5%', fill = "tonexty", fillcolor = "rgba(168, 216, 234, 0.5)")
fig.on <- fig.on  %>% add_trace(x = c(1:(n.days-4)), y = I.count[1:(n.days-4)], mode = "markers", name = 'True prevalence')
fig.on <- fig.on  %>% add_segments(x = 8, xend = 8, y = 0, yend = 450, name = TeX("\\beta_t \\text{ changepoint}"), line = list(color = 'rgb(12, 12, 24)', width = 2, dash = 'dot'))
...
fig.on <- fig.on %>% layout(title = "", legend = list(x=.75, y=.9), xaxis = list(title = "Day", tickfont = list(size = 12)),
                            yaxis = list(title = "Infected", range = c(0,380)), font = t,
                            margin = list(b = 100))
...
#####R_t
true.Rt = beta.t[2:60]/(1/(I.count/pos.count))*S.count/n
...
fig.rt <- fig.rt %>% add_trace(x = c(2:(n.days-4)), y = true.Rt[2:(n.days-4)], mode = "lines", name = 'True R_t', fill = "tonexty", fillcolor = "rgba(168, 216, 234, 0.5)")
