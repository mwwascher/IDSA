...
IDSAplot = function()
{
  ...
  fig.on <- fig.on  %>% add_trace(x = c(1:(n.days-4)), y = I.count[1:(n.days-4)], mode = "markers", name = 'Obesrved prevalence')
  ...
  fig.rt <- fig.rt %>% add_trace(x = c(2:(n.days-4)), y = true.Rt.smooth$y[2:(n.days-4)], mode = "lines", name = 'R_t')
